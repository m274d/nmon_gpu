...
#ifdef SUPPRESS_INTELLISENSE_ERROR
const char* optarg;
#endif
...
#ifdef NVIDIA_GPU
//
// These are actually defined in /usr/local/cuda/include/nvml.h
// but the author could not find the header file.
//
...
nvmlDevice_t gpu_device[MAX_GPU_DEVICES];
nvmlUtilization_t gpu_util[MAX_GPU_DEVICES];
unsigned int gpu_devices;
char gpu_name[MAX_GPU_DEVICES][1024];
unsigned int gpu_temp[MAX_GPU_DEVICES];
unsigned int gpu_watts[MAX_GPU_DEVICES];
unsigned int gpu_clock[MAX_GPU_DEVICES];
nvmlMemory_t gpu_memory[MAX_GPU_DEVICES];
...
void gpu_init()
{
    ...
    if (gpu_devices > MAX_GPU_DEVICES)
	gpu_devices = MAX_GPU_DEVICES;
...
#define LSBLK_STRING "lsblk --nodeps --output NAME,TYPE --raw | grep disk | awk 'BEGIN {printf \"# This file created by: nmon -g auto\\n# It is an automatically generated disk-group file which excluses disk paritions\\n\" } { printf \"%s %s\\n\", $1, $1 }' >auto"
	...
	padgpu = newpad(20, MAXCOLS); // was 10
...
/* DEBUG inject steal       cpu_steal = cpu_sys; */
	...
	if (show_lpar) {
		...
		if (!show_rrd) {
			...
			fprintf(fp, "\n");
			fprintf(fp,
				"GPU_UMEM,GPU Memory Utilization Percent");
			for (i = 0; i < gpu_devices; i++)
			    fprintf(fp, ",GPU%d", i + 1);
			fprintf(fp, "\n");
			fprintf(fp, "GPU_TEMP,GPU Temperature C");
			for (i = 0; i < gpu_devices; i++)
			    fprintf(fp, ",GPU%d", i + 1);
			fprintf(fp, "\n");
			fprintf(fp, "GPU_WATTS,GPU Power Draw Watts");
			for (i = 0; i < gpu_devices; i++)
			    fprintf(fp, ",GPU%d", i + 1);
			fprintf(fp, "\n");
			fprintf(fp, "GPU_MEM,GPU Memory MB");
			for (i = 0; i < gpu_devices; i++)
			    fprintf(fp, ",GPU%d", i + 1);
			fprintf(fp, "\n");
		    }
		    ...
		    for (i = 0; i < gpu_devices; i++)
			fprintf(fp, ",%ld", (long)(gpu_memory[i].used/(1024L*1024L)));
		    fprintf(fp, "\n");
		}
	    }
	}
